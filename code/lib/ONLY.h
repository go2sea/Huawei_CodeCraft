...
struct ONLYNode {
	...
	ONLYNode(int i) : id(i), path_cost(0), already(0), last_v_dis(0) {
		int block = id / 8;
		int offset = id % 8;
	...
	ONLYNode(int i, int c, vector<int> p, unsigned char* p_m, int a, int lvd) : id(i), path_cost(c), path(p), already(a), last_v_dis(lvd) {
...
struct ONLYCompare {
	...
	bool operator () (ONLYNode* node1, ONLYNode* node2) {
		//按经过V中点顺序
...
public:
	...
	long long max_cross_count = 100;
	long long max_pq_size = 30 * 10000;
	int C = 2;
	int max_runtime = 3000;		//最大运行毫秒数
	long long increase_count = 0;
	long long max_increase_count;
	bool only2;	//第二套only方案，1*already 路口点increase clearto
	...
	priority_queue<ONLYNode*, vector<ONLYNode*>, ONLYCompare> pq;	//优先队列（待扩展点） for分支限界
	double average_pheromone;	//平均数
	double media_pheromone;		//中位数
	double Q1_media_pheromone;	//第一4分位数

public:
	ONLY(unordered_map<int, unordered_map<int, int>> nei,
		...
		int city_c)
	{
		...
		//设置max_increase_count
		//7
		if(city_count > 100 && city_count <= 150)
			max_increase_count = 2800000;
		//9
		else if(city_count > 200 && city_count <= 250)
			max_increase_count = 80000;
		//10
		else if(city_count > 250 && city_count <= 300)
			max_increase_count = 50000;
		//12
		else if(city_count > 300 && city_count <=500 && V.size() > 20 && V.size() <= 23)
			max_increase_count = 60000;
	...
	//增加节点路径上所有边的信息素
	void increasePathPhe(ONLYNode* node) {
		if (only2 && node->already == 0)
			return;
		for (int i = 1; i < (int)node->path.size(); i++) {
			//第二套*already
			if(only2)
				pheromone[node->path[i - 1]][node->path[i]] += 1*node->already, increase_count++;
			else
				pheromone[node->path[i - 1]][node->path[i]] += 1, increase_count++;

		}
	}


	bool reachAll(ONLYNode* root) {
		stack<ONLYNode*> stack_node;
		unordered_set<int> visited(root->path.begin(), root->path.end());
		stack_node.push(root);
		while (!stack_node.empty()) {
			ONLYNode* node = stack_node.top();
			stack_node.pop();
			...
			for (auto pair : neighbors[node->id]) {
				...
				ONLYNode* neighbor_node = new ONLYNode(pair.first,
	...
	void addNeighborsToPq(ONLYNode* node) {
		//node的每一条出边
		for (auto pair : neighbors[node->id]) {
			...
			ONLYNode* neighbor_node = new ONLYNode(neighbor_id,
				node->path_cost + cost,
				neighbor_path,
				node->path_map,
			...
			////近似判断是否可达所有V中点
			//if (max_cross_count < 1000 && !reachAll(neighbor_node)) {
			...
			//第一套或者第二套的路口点
			if (!only2 || (only2 && V.count(neighbor_id)))
	...
	//估计合法路径的cost
	int estimateCost(ONLYNode* node) {
	...
	int clearPqTrash() {
		int count = 0;
		priority_queue<ONLYNode*, vector<ONLYNode*>, ONLYCompare> temp_pq;	//优先队列（待扩展点） for分支限界
		while (!pq.empty()) {
			ONLYNode* node = priorityPop();
	...
	void clearHalfPq() {
		...
		for (int i = 0; i < cross_count; i++) {
			temp_pq.push(priorityPop());
		}
		while (!pq.empty()) {
			ONLYNode* temp_node = priorityPop();
			if (V.count(temp_node->id))
				cross_count--;
			delete temp_node;
		}
		pq = temp_pq;
		cout << "清除至 " << cross_count << " 个较优点（可能是非路口点）" << endl;
	...
	//取pq的top并将其pop
	ONLYNode* priorityPop() {
		ONLYNode* result = pq.top();
		pq.pop();
		return result;
	}
	...
	int searchONLY() {
		...
		ONLYNode* source_node = new ONLYNode(source_id);
		pq.push(source_node);
		...
		while (!pq.empty()) {
			...
			node_rest--;
			//只only2清除
			if (only2 && (cross_count >= max_cross_count || pq.size() >= max_pq_size)) {
		...
		//printRuntime();
		//cout << "ONLY完毕！！！" << endl;
		//cout << "increase_count = " << increase_count << endl;
		//system("pause");
		return min_cost;
	}
